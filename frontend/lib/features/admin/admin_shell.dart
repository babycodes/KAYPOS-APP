...
class _AdminShellState extends State<AdminShell> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      drawer: isMobile ? Drawer(child: _buildSidebar(cs, auth, currentPath, true)) : null,
      body: Row(children: [
        if (!isMobile) SizedBox(
          width: sidebarOpen ? 240 : 64,
          child: _buildSidebar(cs, auth, currentPath, false),
        ),
  ...
  Widget _buildSidebar(ColorScheme cs, AuthProvider auth, String currentPath, bool isMobile) {
    return Container(
      ...
      child: Column(children: [
        ...
        Expanded(child: ListView(padding: const EdgeInsets.all(8), children: navItems.map((item) {
          ...
          return Padding(padding: const EdgeInsets.only(bottom: 4), child: InkWell(
            ...
            child: Container(height: 44, padding: const EdgeInsets.symmetric(horizontal: 12),
              ...
              child: Row(children: [
                ...
                if (sidebarOpen || isMobile) ...[const SizedBox(width: 12), Text(item['label'] as String, style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500, color: active ? cs.onPrimaryContainer : cs.onSurfaceVariant))],
              ])),
          ));
        }).toList())),
        const Divider(height: 1),
        ...
        Padding(padding: const EdgeInsets.all(8), child: Column(children: [
          InkWell(onTap: () => context.go('/kasir'), borderRadius: BorderRadius.circular(12),
            child: Container(height: 44, padding: const EdgeInsets.symmetric(horizontal: 12),
              child: Row(children: [
                Icon(Icons.shopping_cart, size: 20, color: cs.primary),
                if (sidebarOpen || isMobile) ...[const SizedBox(width: 12), Text('Ke Kasir', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500, color: cs.primary))],
              ]))),
          InkWell(onTap: () { auth.logout(); context.go('/login'); }, borderRadius: BorderRadius.circular(12),
            child: Container(height: 44, padding: const EdgeInsets.symmetric(horizontal: 12),
              child: Row(children: [
                Icon(Icons.logout, size: 20, color: cs.error),
                if (sidebarOpen || isMobile) ...[const SizedBox(width: 12), Text('Logout', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500, color: cs.error))],
              ]))),
        ])),
      ]),
    );
